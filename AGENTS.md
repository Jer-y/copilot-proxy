...
- **Common targeted tests:** `tests/create-responses.test.ts`, `tests/messages-routing.test.ts`, `tests/model-utils.test.ts`, `tests/copilot-auth-recovery.test.ts`, `tests/auth-recovery-routes.test.ts`, `tests/concurrency-limiter.test.ts`, and `tests/request-signal-regression.test.ts`.
...
- The proxy supports native protocol routes only. Do not translate between Responses, Anthropic Messages, or Chat Completions, or silently switch protocols/models. Reject incompatible combinations with a client-compatible error before contacting Copilot; preserve models that advertise multiple native APIs.
- Model and HTTP endpoint eligibility must come from the selected account's startup-fetched catalog. Do not introduce a static model catalog, family-prefix model lookup, or capability fallback; missing catalogs fail locally and missing endpoint metadata is not eligible. Preserve explicit client limits and use fetched limits for defaults. Keep existing narrow wire-format adaptations separate from model availability.
...
- Supported upstream capabilities should be transparently forwarded. Do not add local explicit rejections solely to handle client compatibility gaps or unknown-but-forwardable fields; prefer transparent forwarding and bounded debug logging for necessary same-protocol Copilot adaptations. Local rejection is still appropriate for malformed requests, security boundaries, or cases where forwarding would create a misleading false success instead of real upstream behavior.
...
- Never refresh/replay structured model, organization, content, or permission `403` responses; local Host/Origin/token-route `403` responses; `429`; 5xx; timeouts; connection resets; or any failure after a 2xx response or downstream stream has begun.
