...
- Treat the proxy as a product boundary with two independent contracts: the client-facing wire contract comes from the current official OpenAI/Anthropic documentation, while the upstream-facing contract comes from dated live Copilot probes for the selected model and endpoint. A correct implementation must satisfy both sides.
...
- Apply this evidence model to reviews, implementation, and tests alike. Record the validation date, account type, model ID, endpoint, request shape, status, and semantic observation for upstream-gated decisions. Classify findings as locally reproduced, official-contract verified, live-upstream verified, client-smoke verified, or conditional/platform-only; do not present one category as another.
...
- Keep official OpenAI `response.create` warmup semantics separate from Copilot behavior. OpenAI defines `generate: false` as a no-output state warmup, but the 2026-07-15 live Copilot probe returned `bad_request` without `input` and generated output when `input` was present. Until a fresh probe proves faithful support, reject `generate: false` locally with `400 unsupported_value` and `param: "generate"` before opening an upstream connection; never report Copilot generation as a successful warmup.
...
- The Codex WSS half must select a model whose live proxy catalog advertises `ws:/responses`. Success requires local and upstream `101` handshake evidence, at least two alternating forwarded/completed turns with matching counts over one active socket, a real local tool execution, and zero `POST /v1/responses` or Codex fallback diagnostics. Codex can return the correct answer after silently falling back to HTTP, so JSONL success and the final sentinel are necessary but insufficient. For changes to connection state or prewarm behavior, additionally capture the redacted `store:false`/`previous_response_id` or `generate:false` evidence described in [the Responses WebSocket smoke guidance](docs/copilot-capability-validation.md#responses-websocket-client-gate).
