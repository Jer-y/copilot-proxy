...
>
> 如果你的目标是给 Claude Code、Codex、SDK 或自定义工具提供一个由 GitHub Copilot 订阅驱动、包含原生 Responses WebSocket 模式的本地 OpenAI / Anthropic 兼容 API 代理，本项目仍然有意义。

---

## 项目概览

这是一个面向 GitHub Copilot API 的逆向代理，把你的 Copilot 订阅暴露为 OpenAI / Anthropic 兼容 HTTP 端点以及原生 Responses WebSocket 模式。你可以用任何支持 OpenAI Chat Completions / Responses 或 Anthropic Messages 的外部工具来调用 GitHub Copilot，包括 [Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview) 与 OpenAI Codex。
...
- **原生 Responses WebSocket 模式**：接受 `GET /v1/responses`（以及 `GET /responses`）的 WebSocket Upgrade，并把每条客户端连接一对一桥接到 Copilot 原生 `wss://.../responses`。只有当前 Copilot 模型元数据明确声明 `ws:/responses` 时才会动态启用，Bun 与 Node.js 均支持；现有 HTTP `POST`/SSE Responses 路径保持可用。
- **Responses SSE/WSS 语义一致性门禁**：提供 opt-in live gate，把同一功能 payload 分别通过真实 HTTP `stream: true` SSE 与 WebSocket `response.create` 发送，并比较可观察语义，而不是只比较状态码。
...
- **有界上游并发**：可通过 `--max-concurrency`、`--max-queue` 与 `--queue-timeout-ms` 限制共享 Copilot 身份的总并发和等待队列。
- **Token 感知自愈**：请求期上游 `401`，或带 GitHub 关联 ID 的纯文本 `403 Forbidden`，会触发一次 single-flight 短期 token 刷新与一次受控重放；新 token 仍被拒绝时会进入冷却熔断，而不是形成刷新风暴。
...
请让 New API 不要对上游 `403` 与 `429` 自动重试。切换到另一个但仍使用同一 Copilot 身份的渠道，依然会命中相同的上游风险桶，并可能放大临时限制。一次受控 token 自愈由 copilot-proxy 负责；恢复熔断打开时，它会直接返回 `503`、`Retry-After` 和 `X-Copilot-Proxy-Recovery-State`，不再向 GitHub 发请求。

共享部署除了 New API 的用户级限流外，还应在 copilot-proxy 设置最后一道身份级并发保护，例如 `--max-concurrency 4 --max-queue 50 --queue-timeout-ms 30000`。这些是本地保护起点，不是 GitHub 公布的服务限额，应按实际负载调整。
...
| --max-queue    | 等待并发槽位的最大请求数（`0` 表示不排队）                              | 50*         | 无   |
| --queue-timeout-ms | 并发队列最长等待毫秒数（`0` 表示不等待）                           | 30000*      | 无   |
...
队列的 `50*` 与 `30000*` 默认值只在 `--max-concurrency` 启用并发限制后生效。`自动*` 表示在 Node.js 运行时，如果没有显式覆盖，发往 `githubcopilot.com` 的请求会默认使用 `900000ms` 响应头 timeout、`900000ms` 响应体 timeout 和 `30000ms` 建连 timeout。其他域名仍沿用 Node/undici 默认值，除非你显式传参覆盖。
...
OpenAI 兼容的 Chat Completions、Models、Embeddings 与 Responses 路由同时接受表中 `/v1/...` 路径和对应的无前缀路径。Anthropic Messages 只在 `/v1/messages` 下提供；`/usage`、`/token`、`/livez` 与 `/readyz` 是无前缀辅助路由。
...
这些端点支持 OpenAI Responses API（`/v1/responses`）。HTTP 请求由 Copilot Responses surface 支持时会直接转发到上游；Claude 模型可以通过 HTTP 路由忠实翻译为 Anthropic Messages。WebSocket 轮次只提供原生直连：首个 `response.create` 选择的 live 模型明确声明 `ws:/responses` 后，代理才会建立一对一 Copilot 桥接；它不会把 Claude/Anthropic Messages、Chat Completions 或 Realtime API 冒充为 WebSocket 支持。

| 端点                 | 方法            | 说明                                                   |
| -------------------- | --------------- | ------------------------------------------------------ |
| `POST /v1/responses` | `POST`          | 创建 Responses API 响应（支持 SSE 流式）               |
| `GET /v1/responses`  | `GET` + Upgrade | 打开 Responses WebSocket；首轮必须选择明确支持的模型   |

WebSocket 模式遵循[官方 Responses WebSocket 契约](https://developers.openai.com/api/docs/guides/websocket-mode)：发送 `response.create` 文本事件，每条连接最多只允许一个响应处于 in-flight，排队请求按 FIFO 串行执行，不做 multiplex。单条连接最长 60 分钟。使用 `store: false` 时，重连后不能继续依赖连接本地缓存的 `previous_response_id`；除非明确使用持久化状态，否则应携带所需完整上下文开始新链。HTTP/SSE 客户端仍可原样使用 `POST /v1/responses`。

该传输中的 streaming 是隐式的。代理会在连接上游之前拒绝 `stream: false` 与非法 `stream` 值；`stream: true` 或 `null` 则作为与传输兼容的 no-op 被剥离，不会转发给 Copilot。

下游输入缓冲与上游并发限制相互独立：每个文本帧最多 16 MiB，每条连接最多排队 8 个轮次或 32 MiB，所有 Responses WebSocket 连接还共享一个 64 MiB 请求缓冲预算，覆盖排队和 setup 阶段的帧。触发任一队列边界时，代理会在本地返回 `429` 并关闭该连接，不会把被拒绝的轮次转发到 Copilot。

兼容性说明：OpenAI 官方 WebSocket 模式把 `generate: false` 定义为不生成输出的状态预热，但 Copilot 在 2026-07-15 的行为并不等价：不带 `input` 时返回 `bad_request`，带 `input` 时反而实际生成输出。因此代理会在连接上游之前 fail-closed，返回 `400 unsupported_value`（`param: "generate"`），而不会把一次真实生成误报成预热成功。
...
| `GET /livez` | GET  | 只检查进程存活，不代表 Copilot 上游可用       |
| `GET /readyz` | GET | 被动返回不含秘密的 token 生命周期、恢复熔断、模型缓存与并发状态；全局熔断未关闭时返回 `503` |
...
`enable` 会拒绝 `_npx`、`pnpm dlx`、`yarn dlx` 和 `bunx` 缓存路径，因为缓存一旦清理，长期服务就会失效。原生自启动必须使用全局安装或稳定的源码 checkout。没有 legacy daemon 配置时，重复执行 `enable` 会保留上一次安装的原生服务配置；显式并发参数会覆盖对应值。若要移除持久化的并发与队列设置，可运行 `copilot-proxy enable --clear-concurrency-limit`。
...
修改 Responses WebSocket 时，先运行以下定向确定性门禁，再跑完整测试：
...
对范围内的每种 Copilot 账号路由运行 live 语义一致性门禁：
...
# 验证企业路由时，改用 COPILOT_ACCOUNT_TYPE=enterprise 再运行一次。
```

门禁对每项功能使用同一 payload，强制 HTTP 侧走真实 SSE，并把 WebSocket 侧封装为 `response.create`。任一 pair 分类不一致或语义校验失败都会让运行失败；两端一致的资源/依赖失败只能记为 inconclusive。要正向验证 `file_search`，还必须设置 `COPILOT_LIVE_VECTOR_STORE_ID` 与 `COPILOT_LIVE_FILE_SEARCH_SENTINEL`。

2026-07-15 的 `gpt-5.4` 实测中，`individual` 与 `enterprise` 两次运行均以 `0` 退出，且各自为 `confirmed=7`、`inconclusive=0`、`failed=0`。Function tool control、`json_object`、`json_schema`、`web_search` 与 `web_search_preview` 在两种传输上都通过语义支持校验；MCP 与 `file_search` 则在两种传输上都得到明确的 capability-unsupported，证明的是拒绝一致性，不代表功能支持。详见[带日期的证据与校验器](docs/copilot-capability-validation.md#responses-ssewss-semantic-parity-gate)。

任何 Responses 行为变更都要运行成对的真实 Codex 客户端门禁；默认先跑 HTTP/SSE，再跑 WSS：
...
脚本的两个半程都会调用机器上安装的真实 `codex` 命令，不会模拟 Codex 客户端。WSS 半程必须证明本地与上游 `101` 握手、没有回退到 HTTP `POST /v1/responses`，并在同一持久连接上完成至少两轮交替的工具调用。详见 [Responses WebSocket 客户端门禁](docs/copilot-capability-validation.md#responses-websocket-client-gate)。
...
- 为避免触发 GitHub Copilot 的速率限制，可使用：
  ...
  - `--max-concurrency <数量>`：限制共享 Copilot 身份的同时上游请求数；`--max-queue` 与 `--queue-timeout-ms` 限制本地等待，溢出时不会触碰 GitHub 上游
- 自动认证恢复是有边界的：代理只刷新 Copilot 短期 token，只在向下游输出任何字节之前重放一次；fresh-token canary 仍被拒绝时会停止重试。不要围绕持续 `403` 构建自动重启或循环刷新机制。
