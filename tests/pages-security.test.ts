...
describe('hosted diagnostics dashboard security', () => {
  ...
  test('does not turn missing diagnostics fields into negative runtime assertions', async () => {
    ...
    expect(html).toContain('const globalConcurrency = isRecord(concurrency.global) ? concurrency.global : concurrency')
    expect(html).toContain('if (globalConcurrency.enabled === false)')
    expect(html).toContain('else if (globalConcurrency.enabled === true)')
