...
describe('build and release supply-chain controls', () => {
  ...
  test('publishes Pages and Docker artifacts only after validation', async () => {
    ...
    expect(release).toMatch(/group: release-\$\{\{ github\.ref \}\}/)
    expect(release.match(/no-cache: true/g)).toHaveLength(3)
    expect(release.match(/package-manager-cache: false/g)).toHaveLength(2)
