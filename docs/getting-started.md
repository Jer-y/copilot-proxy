...
`setup` authenticates when needed, reads the current Copilot model catalog, selects a **direct** route, and probes it through a disposable loopback listener. HTTP eligibility requires the matching endpoint in the fetched `supported_endpoints`; no static model policy is used. Missing endpoint metadata does not qualify a model for setup. Responses WebSocket requires an explicit `ws:/responses` entry. Codex further intersects the HTTP Responses-eligible models with the usable installed bundled entries.
